function [theta_recon, y_recon] = test(z,modes,time_array,states)
theta = states(1,:);
y = states(3,:);
...
for i = 1:modes
    ...
    y_recon = y_recon + 2*real(y_complex*exp(i*j*2*pi.*(time_array)));
end
...
legend('theta', 'reconstructed theta');

figure;
plot(time_array, y);
hold all;
plot(time_array, y_recon);
legend('y', 'reconstructed y');
